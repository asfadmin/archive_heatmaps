...
def edge_crosses_antimeridian(vertex1, vertex2):
    ...
    # True if two vertices are not on the same side of the antimeridian
    ...
    for dir_poly in all_poly:
        ...
        while i < len(dir_poly) - 1:
            ...
            if edge_crosses_antimeridian(dir_poly[i], dir_poly[i + 1]):
                ...
                # m=(y1-y2)/(x1-x2)
                slope = (dir_poly[i][1] - dir_poly[i + 1][1]) / (
                    dir_poly[i][0] - dir_poly[i + 1][0]
                )
                ...
                # Vertical shift to translate our vertex onto the antimeridian
