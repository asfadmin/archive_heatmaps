...
use crate::{
    ingest::{
        async_duckdb::AsyncDuckDBConnection,
        sql::generate_sql,
    },
...
pub async fn request(conn: &AsyncDuckDBConnection, filter: Filter) -> (Vec<Granule>, Vec<Polygon>) {
    ...
    let outline_vec: Vec<Polygon> = conn.query("SELECT geom FROM world_outline;").await.expect("Failed to get world border data").iter().flat_map(|batch| {
        batch.column(0).as_any().downcast_ref::<BinaryArray>().unwrap().iter().flat_map(|wkb_binary| {
                match wkb::reader::read_wkb(wkb_binary.unwrap())
                    .unwrap()
                    .to_geometry() {
                        geo::Geometry::MultiPolygon(multi_poly) => multi_poly.iter().cloned().collect::<Vec<Polygon>>(),
